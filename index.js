	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `[ Valeu meu nobre saiu do grupo pra nossa alegria ❤️  _*${mdata.subject}*_ ] \n*_____________*\n@${num.split('@')[0]}\nÉ us guli☺\n *_____________*\nViadin >_<`
...
client.on('message-new', async (mek) => {
		try {
			...
			mess = {
				wait: '❰ WAIT ❱ *Processando...*',
				asik: '❰ WAIT ❱ *Sendo processado...*',
				success: '❰ ✔️ ❱ *FOOOI...*',
				...
				only: {
					...
					premium: 'Desculpe meu nobre, este pedido é apenas para usuários premium ',
					mod: 'Desculpe, este recurso é para o usuário Mods do Nosso 10',
					benned: 'Você para a banda, por favor, contate o proprietário para abrir sua banda',
					ownerG: '「 *OWNER ONLY* 」Desculpe Sis, este recurso é especificamente para o proprietário do Bot',
					ownerB: '「 *OWNER ONLY* 」Desculpe Sis, este recurso é especial para o proprietário do Bot',
			...
			const ownerNumber = ["551154580970@s.whatsapp.net","551154580970@s.whatsapp.net","551154580970@s.whatsapp.net"]
			const mod = [ownerNumber,"551154580970@s.whatsapp.net","551154580970@s.whatsapp.net","551154580970@s.whatsapp.net"]
			const adminbotnumber = ["551154580970@s.whatsapp.net","551154580970@s.whatsapp.net","551154580970@s.whatsapp.net"]
			const frendsowner = ["551154580970@s.whatsapp.net","551154580970@s.whatsapp.net","551154580970@s.whatsapp.net"]
			const premium = ["551154580970@s.whatsapp.net","551154580970@s.whatsapp.net","551154580970@s.whatsapp.net","551154580970@s.whatsapp.net","551154580970@s.whatsapp.net","551154580970@s.whatsapp.net","551154580970@s.whatsapp.net","551154580970@s.whatsapp.net"]
       ...
       if (!isUser) return reply(mess.only.userB)
                ...
                if (isBanned) return reply(mess.only.benned)
					...
					if (Number(args[0]) === 1) {
						...
						reply('*⟪ SUKSES ⟫ ATIVAR EVENTOS EM GRUPO*')
					} else if (Number(args[0]) === 0) {
						event.splice(from, 1)
						fs.writeFileSync('./database/group/event.json', JSON.stringify(event))
						reply('*⟪ SUKSES ⟫ EVENTO MORTAL EM GRUPO*')
					} else {
						reply(ind.satukos())
					}
					break
...
case 'transfer':
                ...
                addKoinUser('551154580970@s.whatsapp.net', fee)
				...
				if (!isPremium) return reply(mess.only.premium)
				  ...
				  await reply(`LIMIT TRABALHOS AO RESET`)
				break
				...
				break
			       ...
			       case 'valxlove':
					...
					if (isBanned) return reply(mess.only.benned)    
					if (!isUser) return reply(mess.only.userB)
					if (!isPremium) return reply('Você não é um Membro Premium, entre em contato com o proprietário ou digite *${prefix}daftarvip* untuk membeli akses Premium!' ,text, { quoted: mek })
					me = client.user
...
break							             
           ...
           if (!isUser) return reply(mess.only.userB)
            ...
            case 'chatlist':
				...
				if (!isUser) return reply(mess.only.userB)
					if (!isOwner) return reply(mess.only.ownerB)    
					teks = `\`\`\`╭────*「 *TOTAL USUÁRIO DA EQUIPE NOSSO 10👑* 」\n\`\`\``
					no = 0
...
if (isBanned) return reply(mess.only.benned)    
	...
	if (isBanned) return reply(mess.only.benned)    
				...
				reply(mess.wait)
					...
					rano = getRandom('.ogg')
					dtt.length > 600
					? reply('A maior parte do texto é ruim')
	...
	if (!isPublic) return reply(mess.only.publikG)
				...
				if (!isUser) return reply(mess.only.userB)
				if (!isPublic) return reply(mess.only.publikG)     
					const dare =['Envie uma mensagem para o seu ex e diga "eu ainda gosto de você','namorado / namorada agora e ss para jogadores','pap para um membro do grupo','Como "VOCÊ É TÃO BONITA, NÃO É AMIGÁVEL" para caras','ss recente chamada Whatsapp','soltar a emoção 🤥 cada digitação no gc / pc por 1 dia','kirim voice note como posso chamá-lo de baby?','solte a citação da música / citação e marque o membro apropriado para essa citação','pake foto sule sampe 3 hari','ketik pake bahasa daerah 24 jam','ganti nama menjadi "gue anak lucinta luna" selama 5 jam','chat ke kontak wa urutan sesuai %batre kamu, terus bilang ke dia "i lucky to hv you','prank chat mantan dan bilang " i love u, pgn balikan','record voice baca surah al-kautsar','bilang "i hv crush on you, mau jadi pacarku gak?" ke lawan jenis yang terakhir bgt kamu chat (serah di wa/tele), tunggu dia bales, kalo udah ss drop ke sini','sebutkan tipe pacar mu!','snap/post foto pacar/crush','teriak gajelas lalu kirim pake vn kesini','pap mukamu lalu kirim ke salah satu temanmu','kirim fotomu dengan caption, aku anak pungut','teriak pake kata kasar sambil vn trus kirim kesini','teriak " anjimm gabutt anjimmm " di depan rumah mu','ganti nama jadi " BOWO " selama 24 jam','Pura pura kerasukan, contoh : kerasukan maung, kerasukan belalang, kerasukan kulkas, dll']
...
reply(mess.wait)
			...
			if (!isPublic) return reply(mess.only.publikG)
				...
				if (!isUser) return reply(mess.only.userB)
					if (!isOwner) return reply(mess.only.ownerB)    
					teks = `\`\`\`╭────*「 *TOTAL USER TROPA NOSSO 10 👑* 」\n\`\`\``
					no = 0
					for (let hehehe of user) {
						no += 1
						teks += `\`\`\`[${no.toString()}]\`\`\` @${hehehe.split('@')[0]}\n`
    ...
    client.sendMessage(from, options, text, { quoted: mek } )
		...
		if (args.length < 1) return reply('tag temanmu!')
			...
			break
				...
				if (!isUser) return reply(mess.only.userB)
                reply('Otw.....')
                                       if (args[0].startsWith('08')) return reply('Use números de prefixo 8/n ex : *8796662*')
     ...
     if (!isUser) return reply(mess.only.userB)
     if (!isPublic) return reply(mess.only.publikG)
     if (args.length < 1) return reply(from, 'Enviar pedidos *!clima [tempat]*\nContoh : *!cuaca Banyuwangi', text)
     reply(mess.wait)
 ...
 if (isBanned) return reply(mess.only.benned)    
		...
		;			client.sendMessage(from, thumb, image, {quoted: mek, caption: teks})
			...
			if (isBanned) return reply(mess.only.benned)    
				...
				if (!isUser) return reply(mess.only.userB)
				if (!isPublic) return reply(mess.only.publikG)
					if (args.length < 1) return reply('Insira o nome do motor!!')
					ft1 = body.slice(11)
					reply(mess.wait)
